...
resource "aws_subnet" "private-subnet-08" {
  depends_on = [
    aws_vpc.main
  ]
  vpc_id            = aws_vpc.main.id
  cidr_block        = var.private-subnets-cdir[3]
  availability_zone = var.aws_availability_zones[3]
  tags = merge(var.common_tags, {
    Name                                        = format("%s-%s-%s-private-08-${var.aws_availability_zones[3]}", var.common_tags["id"], var.common_tags["environment"], var.common_tags["project"])
    "kubernetes.io/role/internal-elb"           = "1"
    "kubernetes.io/cluster/${var.cluster_name}" = var.shared
    },
  )
